...
function generateTimetable() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        yield prisma.timetableSlot.deleteMany();
        const school = yield prisma.school.findFirst();
        if (!school)
            throw new Error("School config not found");
        const workingDays = JSON.parse(school.workingDays);
        // E.g., "08:00"
        ...
        let leisureSub = yield prisma.subject.findFirst({ where: { name: 'Leisure' } });
        if (!leisureSub) {
            leisureSub = yield prisma.subject.create({ data: { name: 'Leisure', category: 'Misc', duration: 40, requiresLab: false } });
        }
        let loungeClass = yield prisma.class.findFirst({ where: { section: 'LOUNGE' } });
        if (!loungeClass) {
            loungeClass = yield prisma.class.create({ data: { grade: 0, section: 'LOUNGE' } });
        }
        subjects.push(leisureSub);
        const getSubjectsForClass = (grade) => {
            const sEnglish = subjects.find((s) => s.name === 'English');
            const sHindi = subjects.find((s) => s.name === 'Hindi');
            const sMaths = subjects.find((s) => s.name === 'Maths');
            const sSocialScience = subjects.find((s) => s.name === 'Social Science');
            const sScience = subjects.find((s) => s.name === 'Science');
            const sIT = subjects.find((s) => s.name === 'IT');
            const sComputer = subjects.find((s) => s.name === 'Computer');
            const sPunjabi = subjects.find((s) => s.name === 'Punjabi');
            const sSanskrit = subjects.find((s) => s.name === 'Sanskrit');
            const sPhysics = subjects.find((s) => s.name === 'Physics');
            const sChemistry = subjects.find((s) => s.name === 'Chemistry');
            const sBiology = subjects.find((s) => s.name === 'Biology');
            const sCompScience = subjects.find((s) => s.name === 'Computer Science');
            const sGames = subjects.find((s) => s.name === 'Games');
            const sYoga = subjects.find((s) => s.name === 'Yoga');
            const sMusic = subjects.find((s) => s.name === 'Music');
            const sDance = subjects.find((s) => s.name === 'Dance');
            if (grade >= 1 && grade <= 5) {
        ...
        const getSubjectDemandCount = (grade, subId) => {
            ...
            if (grade >= 1 && grade <= 5) {
                ...
                if (s.name === 'IT')
                    return 3;
                if (s.name === 'Computer')
                    return 2; // Reduced to 2 to make room for Dance (Total 40 blocks)
                if (s.name === 'Games' || s.name === 'Yoga' || s.name === 'Music' || s.name === 'Dance')
                    return 1;
            }
            else if (grade >= 9 && grade <= 10) {
                if (s.name === 'Physics' || s.name === 'Chemistry' || s.name === 'Biology' || s.name === 'Computer Science')
                    return 2; // 80m blocks
                if (s.name === 'English' || s.name === 'Maths' || s.name === 'Hindi' || s.name === 'Social Science')
                    return 5;
                if (s.name === 'Games' || s.name === 'Yoga' || s.name === 'Music' || s.name === 'Dance')
                    return 1;
            }
        ...
        const busyMap = new Set(); // "teacherId-day-blockIdx" or "labId-day-blockIdx"
        const markBusy = (teacherId, labId, day, startBlock, blocks, busy) => {
        ...
        const isFree = (teacherId, labId, day, startBlock, blocks) => {
            for (let i = 0; i < blocks; i++) {
                const bIdx = startBlock + i;
                if (busyMap.has(`t-${teacherId}-${day}-${bIdx}`))
                    return false;
                if (labId && busyMap.has(`l-${labId}-${day}-${bIdx}`))
                    return false;
            }
            return true;
        };
        ...
        const regularClasses = classes.filter((c) => c.section !== 'LOUNGE');
        ...
        for (const cls of sortedClasses) {
            ...
            for (const sub of clsSubjects) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    const eligibleTeachers = teachers.filter((t) => t.subjects.some((ts) => ts.subjectId === sub.id) &&
                    ...
                    if (sub.requiresLab) {
                        const eligibleLabs = labs.filter((l) => l.type === sub.labType);
        ...
        let nodesExplored = 0;
        let deepestClass = 0;
        function solveClass(cIndex, maxBlocksPerDay) {
            if (cIndex === sortedClasses.length)
                return true; // Done!
            if (cIndex > deepestClass)
                deepestClass = cIndex;
        ...
        function packEvents(cls, remainingEvents, maxBlocksPerDay) {
            nodesExplored++;
            if (nodesExplored > 10000)
                return false; // Fail fast per search node to allow restarts/relaxations
            ...
            for (let startBlock = 0; startBlock <= 40 - blocksNeeded; startBlock++) {
                if (nodesExplored > 10000)
                    return false; // Abort loop if node limit reached
                const day = Math.floor(startBlock / 8) + 1;
                const timeWithinDayBlocks = startBlock % 8;
                if (timeWithinDayBlocks + blocksNeeded > 8)
                    continue; // Crosses day boundary
                // NO LUNCH OVERLAP
                if (blocksNeeded === 2 && timeWithinDayBlocks === 5)
                    continue;
                ...
                if (subjectBlocksToday + blocksNeeded > maxBlocksPerDay)
                    continue; // CAP
                ...
                if (blocksNeeded === 2) {
                    ...
                    if (sub) {
                        if (sub.name === 'Physics' && day === 5)
                            continue; // Try to keep off Day 5
                        if (sub.name === 'Chemistry' && day === 3)
                            continue;
                        if (sub.name === 'Biology' && day === 1)
                            continue;
                        if (sub.name === 'Computer Science' && day === 2)
                            continue;
                    }
                }
                // Check if teacher/lab is free
                if (!isFree(ev.teacherId, ev.labId, day, timeWithinDayBlocks, blocksNeeded))
                    continue;
                // Place
        ...
        for (let attempt = 1; attempt <= 1000; attempt++) {
            ...
            if (elapsed > 20000)
                maxBlocksPerDay = 3; // Loosen to 3 if hitting timeout to guarantee generation
            if (elapsed > 27000)
                maxBlocksPerDay = 8; // Complete fallback
            if (attempt % 50 === 1) {
        ...
        if (success) {
            ...
            yield prisma.timetableSlot.createMany({
                data: generatedSlots
            });
